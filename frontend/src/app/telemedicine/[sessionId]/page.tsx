...
interface TelemedicineSessionViewModel {
    sessionId: string;
    appointmentId: string;
    doctorName: string;
    patientName: string;
...
export default async function TelemedicineSessionPage({ params }: { params: Promise<{ sessionId: string }> }) {
    ...
    let sessionData: TelemedicineSessionViewModel | null = null;
    ...
    try {
        ...
        if (res.ok) {
            const backendSession = (await res.json()) as BackendSessionResponse;

            const scheduledAt = backendSession.startedAt ?? new Date().toISOString();
            const status: SessionStatus = backendSession.endedAt ? "COMPLETED" : "STARTED";

            sessionData = {
                sessionId: backendSession.id,
                appointmentId: backendSession.appointmentId,
                doctorName: "Assigned Doctor",
                patientName: "Assigned Patient",
                scheduledAt,
                status,
                meetingUrl: backendSession.jitsiUrl,
            };
        } else {
            fetchError = true;
    ...
    return (
        <div className="min-h-screen" style={{ background: "var(--bg-surface)" }}>
            <div className="max-w-300 mx-auto px-4 py-8 space-y-5">
                {/* Session info bar */}
                <div
                ...
                >
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-0.5 mt-0.5 text-xs" style={{ color: "var(--text-muted)" }}>
                                ...
                                <span>Appointment: {sessionData.appointmentId}</span>
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                        ...
                        {sessionData.status === "SCHEDULED" || sessionData.status === "STARTED" ? (
                            <EndSessionButton sessionId={sessionData.sessionId} appointmentId={sessionData.appointmentId} />
                        ) : null}
                        <Link
                            href="/dashboard"
