...
function makeMockSlot(overrides = {}) {
  return {
    ...
    doctorId: new Types.ObjectId(mockId),
    dayOfWeek: 'Monday',
    startTime: '09:00',
    endTime: '13:00',
    isActive: true,
...
describe('DoctorService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: AppointmentIntegrationService,
          useValue: mockAppointmentIntegration,
        },
        {
  ...
  describe('create', () => {
    ...
    it('throws ConflictException when email already exists', async () => {
      ...
      await expect(
        service.create({ name: 'X', email: 'test@example.com' } as any),
  ...
  describe('findOne', () => {
    it('returns a doctor with availability', async () => {
      mockDoctorModel.findById.mockReturnValue({
        lean: () => Promise.resolve(makeMockDoctor()),
      });
      mockAvailabilityModel.find.mockReturnValue({
  ...
  describe('update', () => {
    it('updates a doctor profile', async () => {
      mockDoctorModel.findByIdAndUpdate.mockResolvedValue(
        makeMockDoctor({ specialization: 'Neurology' }),
      );
    ...
    it('throws NotFoundException when doctor not found', async () => {
      ...
      await expect(service.update(mockId, {} as any)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('verify', () => {
    ...
    it('throws BadRequestException if already verified', async () => {
      mockDoctorModel.findById.mockResolvedValue(
        makeMockDoctor({ isVerified: true }),
      );
  ...
  describe('addAvailability', () => {
    ...
    it('throws ConflictException on overlapping slot', async () => {
      mockAvailabilityModel.findOne.mockReturnValue({
        lean: () => Promise.resolve(makeMockSlot()),
      });
      await expect(service.addAvailability(mockId, dto as any)).rejects.toThrow(
        ConflictException,
      );
    });
    it('throws BadRequestException when startTime >= endTime', async () => {
      await expect(
        service.addAvailability(mockId, {
          ...dto,
          startTime: '13:00',
          endTime: '09:00',
        } as any),
  ...
  describe('toggleAvailability', () => {
    ...
    it('throws NotFoundException for unknown slot', async () => {
      ...
      await expect(service.toggleAvailability(mockId, mockId)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
